...
{
	/// <summary>
	/// Application context to use when starting a <see cref="TitleBarTabMainForm" /> application via <see cref="Application.Run(ApplicationContext)" />.  Used to
	...
	public class TitleBarTabsApplicationContext : ApplicationContext
	{
		...
		protected List<TitleBarTabMainForm> _openWindows = new List<TitleBarTabMainForm>();

		/// <summary>List of all opened windows.</summary>
		public List<TitleBarTabMainForm> OpenWindows
		{
			get
			{
				return _openWindows;
		...
		{
			_openWindows.Remove((TitleBarTabMainForm) sender);
