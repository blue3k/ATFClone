...
namespace Sce.Atf.Gui.TitleBarTab
{
	...
	/// </summary>
	public abstract partial class TitleBarTabMainForm : Form
	{
		/// <summary>
		/// Event delegate for <see cref="TitleBarTabMainForm.TabDeselecting" /> and <see cref="TitleBarTabMainForm.TabSelecting" /> that allows subscribers to cancel the
		...
		/// <summary>Event delegate for <see cref="TitleBarTabMainForm.TabSelected" /> and <see cref="TitleBarTabMainForm.TabDeselected" />.</summary>
		/// <param name="sender">Object for which this event was raised.</param>
		/// <param name="e">Data associated with the event.</param>
		...
		/// </summary>
		protected TitleBarTabItem _previousActiveTab = null;
		...
		protected ListWithEvents<TitleBarTabItem> _tabs = new ListWithEvents<TitleBarTabItem>();

		/// <summary>Default constructor.</summary>
		protected TitleBarTabMainForm()
		{
		...
		public bool AeroPeekEnabled
		{
			...
			set
			{
				...
				if (!_aeroPeekEnabled)
				{
					foreach (TitleBarTabItem tab in Tabs)
					{
		...
		/// <summary>List of tabs to display for this window.</summary>
		public ListWithEvents<TitleBarTabItem> Tabs
		{
			get
			{
				return _tabs;
		...
		public TitleBarTabItem SelectedTab
		{
			get
			{
				return Tabs.FirstOrDefault((TitleBarTabItem t) => t.Active);
			}
		...
		public int SelectedTabIndex
		{
			get
			{
				return Tabs.FindIndex((TitleBarTabItem t) => t.Active);
			}

			set
			{
				TitleBarTabItem selectedTab = SelectedTab;
		...
		/// <summary>
		/// Callback that should be implemented by the inheriting class that will create a new <see cref="TitleBarTabItem" /> object when the add button is
		/// clicked.
		/// </summary>
		/// <returns>A newly created tab.</returns>
		public abstract TitleBarTabItem CreateTab();
		...
		protected void UpdateTabThumbnail(TitleBarTabItem tab)
		{
		...
		/// <summary>
		/// Handler method that's called when Aero Peek needs to display a thumbnail for a <see cref="TitleBarTabItem" />; finds the preview bitmap generated in
		...
		{
			foreach (
				TitleBarTabItem rdcWindow in Tabs.Where(rdcWindow => rdcWindow.Content.Handle == e.WindowHandle && _previews.ContainsKey(rdcWindow.Content)))
			{
		...
		/// <summary>
		/// Callback for the <see cref="Control.ClientSizeChanged" /> event that resizes the <see cref="TitleBarTabItem.Content" /> form of the currently selected
		...
		/// <summary>Resizes the <see cref="TitleBarTabItem.Content" /> form of the <paramref name="tab" /> to match the size of the client area for this window.</summary>
		/// <param name="tab">Tab whose <see cref="TitleBarTabItem.Content" /> form we should resize; if not specified, we default to
		/// <see cref="SelectedTab" />.</param>
		public void ResizeTabContents(TitleBarTabItem tab = null)
		{
			if (tab == null)
			{
				tab = SelectedTab;
		...
		{
			foreach (TitleBarTabItem tab in Tabs.Where(tab => tab.Content.Handle == e.WindowHandle))
			{
		...
		protected virtual TabbedThumbnail CreateThumbnailPreview(TitleBarTabItem tab)
		{
			TabbedThumbnail preview = TaskbarManager.Instance.TabbedThumbnail.GetThumbnailPreview(tab.Content);
		...
		public virtual void UpdateThumbnailPreviewIcon(TitleBarTabItem tab, Icon icon = null)
		{
			if (!AeroPeekEnabled)
			{
				return;
		...
		/// Aero Peek preview.
		/// </summary>
		/// <param name="sender">Object from which this event originated (the <see cref="TitleBarTabItem.Content" /> object in this case).</param>
		/// <param name="e">Arguments associated with the event.</param>
		...
		/// <summary>
		/// Event handler that is called when a tab's <see cref="TitleBarTabItem.Closing" /> event is fired, which removes the tab from <see cref="Tabs" /> and
		/// re-renders <see cref="_overlay" />.
		/// </summary>
		/// <param name="sender">Object from which this event originated (the <see cref="TitleBarTabItem" /> in this case).</param>
		/// <param name="e">Arguments associated with the event.</param>
		private void TitleBarTabs_Closing(object sender, CancelEventArgs e)
		{
			TitleBarTabItem tab = (TitleBarTabItem) sender;
			CloseTab(tab);
		...
		{
			TitleBarTabItem newTab = CreateTab();
		...
		protected virtual void CloseTab(TitleBarTabItem closingTab)
		{
